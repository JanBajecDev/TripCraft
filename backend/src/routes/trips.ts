...
import { trips, messages, itinerary } from '../db/schema'
...
router.get('/:id', async (c) => {
  ...
  const itin = await db.query.itinerary.findFirst({ where: eq(itinerary.tripId, id) })
...
router.post('/:id/messages', async (c) => {
  ...
  await db.insert(messages).values({ tripId, role: 'user', content: userMessage })
