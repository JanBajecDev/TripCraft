...
export default function App() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const msg = `Plan my trip to ${dest?.city ?? intake.destination} — ${intake.tripDays} days, ${intake.travellers} ${intake.travellers === 1 ? 'person' : 'people'}, budget £${intake.budgetGbp.toLocaleString()}, dates ${intake.dateLabel}. Interests: ${intake.interests.join(', ')}.`
      setTripId(id)
  ...
  return (
    <div className="app">
      {error && (
        <div style={{ padding: '12px 20px', background: 'var(--error-container)', color: 'var(--on-error-container)', fontSize: 13 }}>
          {error}
        </div>
      )}
      <IntakePage
